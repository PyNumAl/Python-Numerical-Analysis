...
def fjac(f, x, h=None, method=2, args=None):
    """
    Approximate the jacobian of a real-valued vector function using real or complex-step finite differences.
    ...
    --------
    f : callable
        Real-valued vector function whose jacobian is to be approximated.
        
    x : array_like
        Real-valued vector on which to approximate the jacobian.
        
    h : int, float, or complex scalar or array_like, optional
        The stepsize to use for jacobian approximation. Simply pass in complex numbers to use
        complex-step differentiation.
    
    method : {1, 2, 4}, optional
        The finite difference method to use for approximation of the jacobian. 
    ...
    """
    f, fx, H = prepare_input_fjac(f, x, h, method, args)
    n = len(fx)
    J = np.zeros((n,n))
    for i in range(n):
        if H.dtype == complex:
            ...
            if method==1:
                ...
                J[:,i] = np.matmul([1, -8, 0, 8, -1],
                                   [f(x - 2*H[i]), f(x - H[i]), fx,
                                    f(x + H[i]), f(x + 2*H[i])]
                                   ) / (12*H[i,i])
    return J

# Vectorized jacobian function
vfjac = np.vectorize(fjac, signature='(n)->(n,n)', excluded=[0, 2, 3, 4])

def prepare_input_fjac(f, x, h, method, args):
    if args is not None:
    ...
    fx = np.asarray(f(x))
    if fx.ndim!=1:
        raise ValueError('Output of f must be a 1d vector.')
    elif fx.ndim==1 and len(fx)!=len(x):
        raise ValueError(f'Incompatible lengths for output of f and input x, {fx.size} and {x.size}.')
    elif fx.dtype not in [int, float]:
        raise TypeError('Invalid output type for f. f is expected to be a real-valued vector function for real-valued vector inputs.')
    else:
        pass
    return f, fx, H